...
func retornarReloj(nombreArchivo string) (string, error) {
	...
	archivo, err := os.Open(filepath.Join(dirActual, "Fulcrum1", nombreArchivo))
	if err != nil {
		return "", err
	}
	defer archivo.Close()
...
func editarReloj(nombreArchivo string) error {
	...
	contenidos, err := ioutil.ReadFile(filepath.Join(dirActual, "Fulcrum1", nombreArchivo))
	if err != nil {
		return err
	}
...
func (s *baseServiceServer) CrearRegistro(sectorFileName string) error {
	...
	file, err := os.Create(filepath.Join(dirActual, "Fulcrum1", sectorFileName))
	if err != nil {
		return err
	}
	file.Close()
...
func (s *baseServiceServer) AgregarLOG(info, sector, base, nuevonombre string, valor, nuevovalor float32) (*pb.Respuesta, error) {
	...
	if _, err := os.Stat(filepath.Join(dirActual, "Fulcrum1", "Registro.txt")); os.IsNotExist(err) {
	...
	logfile, err := os.OpenFile(filepath.Join(dirActual, "Fulcrum1", "Registro.txt"), os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
...
func (s *baseServiceServer) AgregarBase(ctx context.Context, req *pb.AgregarBaseRequest) (*pb.Respuesta, error) {
	...
	if _, err := os.Stat(filepath.Join(dirActual, "Fulcrum1", nombreArchivo)); os.IsNotExist(err) {
		...
		file, err := os.OpenFile((filepath.Join(dirActual, "Fulcrum1", nombreArchivo)), os.O_APPEND|os.O_WRONLY, 0644)
		if err != nil {
...
func (s *baseServiceServer) RenombrarBase(ctx context.Context, req *pb.RenombrarBaseRequest) (*pb.Respuesta, error) {
	...
	if _, err := os.Stat(filepath.Join(dirActual, "Fulcrum1", nombreArchivo)); os.IsNotExist(err) {
		...
		contenido, err := ioutil.ReadFile(filepath.Join(dirActual, "Fulcrum1", nombreArchivo))
		if err != nil {
			return &pb.Respuesta{Mensaje: "Comando RenombrarBase no pudo ser ejecutado", Exitoso: false}, err
		}
		...
		err = ioutil.WriteFile(filepath.Join(dirActual, "Fulcrum1", nombreArchivo), []byte(nuevoContenido), 0644)
		if err != nil {
			return &pb.Respuesta{Mensaje: "Comando RenombrarBase no pudo ser ejecutado", Exitoso: false}, err
		}
		// Editar reloj
...
func (s *baseServiceServer) GetSoldados(ctx context.Context, req *pb.GetSoldadosRequest) (*pb.Respuesta, error) {
	...
	data, err := ioutil.ReadFile(filepath.Join(dirActual, "Fulcrum1", nombreArchivo))
	if err != nil {
		return &pb.Respuesta{
